...
class SignalTime:
    ...
    def to_spectrogram(self, window: str, nperseg: int, poverlap: float) -> SignalSpectrogram:
        noverlap=int(poverlap * nperseg)
        zero_mean_signal=self.signal - np.mean(self.signal)
        f, t, X=spectrogram(
            zero_mean_signal,
...
class SignalDCT:
    ...
    def to_time(self) -> SignalTime:
        ...
        self.is_compressed=True if self.original_length > len(
            self) else False
...
class Sample:
    ...
    def __post_init__(self):
        year, month, day, hour, minute=[
            int(x) for x in self.sample_id.split("_")]
